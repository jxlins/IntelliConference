    ...
    public List<AuthorCandidate> buildCandidates(List<OpenAlexPaper> papers, int maxAuthors) {
        ...
        for (OpenAlexPaper paper : papers) {
            for (OpenAlexAuthor author : paper.getAuthors()) {
                ...
                // 有机构信息时按「姓名+机构」聚合，可合并 OpenAlex / Semantic Scholar 中的同一位作者；
                // 无机构信息时退回作者 ID，避免同名不同人被错误合并。
                String key = StrUtil.isNotBlank(author.getInstitution())
                        ? normalizedName + "|" + author.getInstitution().toLowerCase()
                        : StrUtil.blankToDefault(author.getId(), normalizedName);
        ...
        return candidateMap.values().stream()
                ...
                .sorted((left, right) -> {
                    int corresponding = Boolean.compare(right.isCorresponding(), left.isCorresponding());
                    if (corresponding != 0) return corresponding;
                    int recent = Integer.compare(right.getRecentPaperCount(), left.getRecentPaperCount());
                    if (recent != 0) return recent;
                    int papersCount = Integer.compare(right.getPaperCount(), left.getPaperCount());
                    if (papersCount != 0) return papersCount;
                    return Integer.compare(right.getTotalCitations(), left.getTotalCitations());
                })
