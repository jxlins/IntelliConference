...
public class AuthorIdentityMatcher {
    ...
    public double score(AuthorCandidate candidate, EmailEvidence evidence) {
        ...
        // A shared paper title/affiliation alone cannot identify which coauthor owns an email.
        if (StrUtil.isNotBlank(evidence.getEmail())
                && !localPartContainsNamePart(emailLocalPart, nameParts)
                && !localPartOverlap(emailLocalPart, nameParts)
                && !(StrUtil.isNotBlank(normalizedName) && evidenceText.contains(normalizedName))) return 0.30;
